...
function Banner() {
  ...
  return (
    < >
      ...
      <motion.div
      className=" flex justify-center items-center md:-mx-20 lg:-mx-28 ">
        <Swiper
          direction="vertical"
          spaceBetween={0}
          slidesPerView={1}
          ...
          // mousewheel={true}
          className=" rounded-3xl w-full xl:h-96 bg-transparent lg:h-64 h-48"
        >
          {trending.map((anime) => (
            <SwiperSlide
            ...
            >
              ...
              <div className="absolute bg-black bg-opacity-40 rounded-3xl backdrop-blur-md w-1/3 max-sm:w-1/2  md:text-xl items-center lg:text-3xl font-extrabold top-0 bottom-0 right-0 left-0 px-5 grid justify-center grid-rows-3">
                <div className=" w-full flex justify-center ">{anime.title.romaji}</div>
      ...
      <div
       className=" transition duration-700 font-rubik text-center sm:text-xl md:text-3xl lg:text-5xl tracking-wider max-sm:pt-8 md:pt-12 lg:pt-16  mx-auto flex justify-center items-center">
          Experience    The    Best    Anime    Streaming    on    The    Web
        ...
        <div
          className=" transition-all h-32 w-16 hover:h-36  max-sm:w-28 max-sm:h-28 bg-black bg-opacity-50 rounded-full flex justify-center items-center text-black hover:text-white duration-500 ">
             <HiArrowNarrowDown className=' text-5xl' />
          </div>
        </AnimationOnScroll>
        </NavHashLink>
        </div>
