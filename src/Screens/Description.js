  ...
  return (
    <>
      <div className= {` h-screen text-white flex items-center justify-center ` }
          style={{backgroundImage: `url(${info.image})` , backgroundSize: "cover" , backgroundPosition: 'center' , backgroundRepeat: 'no-repeat' , backgroundAttachment: 'fixed'}}
      >
      {loading ? <>
      ...
      <div className=' text-4xl font-rubik font-thin'>{info.title.romaji}</div>
      <div>{info.description}</div>
      <div className=' text-left mx-auto'>
        <p><span className='bg-black bg-opacity-20 rounded-md px-2 text-submain font-extrabold '>Genre</span> : {info.genres+" "}</p>
        <p><span className=' bg-black bg-opacity-20 rounded-md px-2 text-submain  font-extrabold '>Episodes</span> : {info.episodes.length}</p>
        <p><span className=' bg-black bg-opacity-20 rounded-md px-2 text-submain  font-extrabold '>Ratings</span> : {info.rating/10}</p>
      </div>
      ...
      <button
        ...
        onClick={() => callPlayer(info.episodes)}
      >
        <img
